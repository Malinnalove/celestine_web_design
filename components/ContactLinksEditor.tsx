...
type IconName = "facebook" | "youtube" | "instagram" | "twitter" | "x" | "linkedin" | "github" | "link" | "website";
...
const iconMap: Record<IconName, JSX.Element> = {
  ...
  link: (
    <svg viewBox="0 0 24 24" className="h-4 w-4 stroke-current" fill="none" strokeWidth="1.6">
                ...
                <>
                  <p className="flex items-center gap-2 text-xs uppercase tracking-[0.3em] text-ink/50">
                    {renderIcon(contact.icon) && (
                      <span className="flex h-7 w-7 items-center justify-center rounded-full bg-ink/5 text-ink">
                        {renderIcon(contact.icon)}
                      </span>
                    )}
                    {contact.label}
